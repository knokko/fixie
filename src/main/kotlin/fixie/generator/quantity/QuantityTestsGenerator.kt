...
abstract class QuantityTestsGenerator<T : QuantityClass>(
	...
	protected fun generateFixedNearlyEquals(number: NumberClass, oneUnitName: String) {
		val baseToDoubleParameters =
			if (quantity.getSupportedUnits()[0].enumName.isNotEmpty()) "${quantity.getSupportedUnits()[0].enumName}.$oneUnitName" else ""
	...
	protected open fun generateCompareToBody() {
		...
		for ((index, unit) in units.withIndex()) {
			if (index != 0) {
				...
				// Annoying edge case where meter and yard are exactly the same
				if (previousUnit.name == "METER" && unit.name == "YARD" && unit.minDelta > 0.05) continue
