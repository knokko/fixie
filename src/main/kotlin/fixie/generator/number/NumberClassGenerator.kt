...
internal class NumberClassGenerator(
	...
	private fun generateClassPrefix() {
		...
		writer.println(
			"value class ${number.className} private constructor(val raw: ${number.internalType.className}) : " +
	...
	private fun generateToString() {
		...
		if (maybe10Power != null) {
			...
			val nextPower =
				if (power * BigInteger.TEN <= BigInteger.valueOf(Long.MAX_VALUE)) "BigInteger.valueOf(${power * BigInteger.TEN})"
	...
	private fun generateStandardTypesArithmetic(methodName: String, operator: String) {
		for (otherType in arrayOf("Int", "Long", "Float", "Double")) {
			...
			if ((otherType == "Int" || otherType == "Long") && (operator == "*" || operator == "/")) {
				...
				val right = if (number.internalType.signed) "right" else "right.to${
					IntType(
						false,
						max(numOtherBytes, number.internalType.numBytes)
					)
				}()"
				var raw = "raw"
				if (number.internalType.numBytes < numOtherBytes) raw =
					"raw.to${IntType(number.internalType.signed, numOtherBytes)}()"
				...
				if (number.checkOverflow) {
					...
					if (number.internalType.numBytes < numOtherBytes) function =
						"($function).to${number.internalType}()"
	...
	private fun generateTimesSelf() {
		...
		if (number.internalType.numBytes == 8) {
			...
			if (exponent != null) {
				...
				if (number.checkOverflow) {
					val conversion =
						if (number.internalType.signed) "result.longValueExact()" else "bigIntegerToULong(result, true)"
					generateTryCatch(
						"\t\t\t", listOf(
					...
					val conversion =
						if (number.internalType.signed) "result.toLong()" else "bigIntegerToULong(result, false)"
	...
	private fun generateFromIntType(intType: IntType) {
		...
		if (intType.numBytes > number.internalType.numBytes) {
			...
			if (!number.checkOverflow || (cantUnderflow && cantOverflow)) {
				...
				writer.println("\t\tfun from(value: $intType): ${number.className} {")
				val toInternalType =
					if (intType.numBytes < 8 && number.internalType.numBytes == 8) ".to${number.internalType}()" else ""
				generateTryCatch(
					"\t\t\t", listOf(
	...
	private fun generateFromFloatAndDouble() {
		...
		var toFunction =
			if (number.internalType.signed && number.internalType.numBytes == 8) "" else ".to${number.internalType}()"
