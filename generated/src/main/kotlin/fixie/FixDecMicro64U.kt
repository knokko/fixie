...
value class FixDecMicro64U private constructor(val raw: ULong) : Comparable<FixDecMicro64U> {
	...
	@Throws(FixedPointException::class)
	operator fun times(right: Int): FixDecMicro64U {
		if (right < 0) throw FixedPointException("Negative numbers are not supported")
		try {
			return FixDecMicro64U(multiplyExact(raw, right.toULong()))
		} catch (overflow: ArithmeticException) {
	...
	@Throws(FixedPointException::class)
	operator fun times(right: Long): FixDecMicro64U {
		if (right < 0) throw FixedPointException("Negative numbers are not supported")
		try {
			return FixDecMicro64U(multiplyExact(raw, right.toULong()))
		} catch (overflow: ArithmeticException) {
	...
	@Throws(FixedPointException::class)
	operator fun div(right: Int): FixDecMicro64U {
		if (right <= 0) {
			throw FixedPointException("Can't represent $this / $right")
		}
		return FixDecMicro64U(raw / right.toULong())
	}

	@Throws(FixedPointException::class)
	operator fun div(right: Long): FixDecMicro64U {
		if (right <= 0L) {
			throw FixedPointException("Can't represent $this / $right")
		}
		return FixDecMicro64U(raw / right.toULong())
	...
	operator fun compareTo(other: Long) = if (other < 0) 1 else if (other > 18446744073709) -1 else this.compareTo(from(other))

	operator fun compareTo(other: ULong) = if (other > 18446744073709u) -1 else this.compareTo(from(other))
	operator fun compareTo(other: Float) = if (other < 0.0f) 1 else if (other > 1.8446744E13f) -1 else this.compareTo(from(other))
	operator fun compareTo(other: Double) = if (other < 0.0) 1 else if (other > 1.844674407370955E13) -1 else this.compareTo(from(other))
	...
	companion object {
		...
		fun from(value: UInt) = FixDecMicro64U(value.toULong() * RAW_ONE)

		@Throws(FixedPointException::class)
		fun from(value: ULong): FixDecMicro64U {
			try {
				return FixDecMicro64U(multiplyExact(value, RAW_ONE))
			} catch (overflow: ArithmeticException) {
				throw FixedPointException("Can't represent $value")
...
@Throws(FixedPointException::class)
operator fun Int.times(right: FixDecMicro64U) = right * this

@Throws(FixedPointException::class)
operator fun Long.times(right: FixDecMicro64U) = right * this

@Throws(FixedPointException::class)
operator fun Float.times(right: FixDecMicro64U) = right * this

@Throws(FixedPointException::class)
operator fun Double.times(right: FixDecMicro64U) = right * this
