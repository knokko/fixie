...
fun parseModule(document: GddlDocument) = ModuleParser(document).result
...
private class ModuleParser(private val document: GddlDocument) {
    ...
    private fun requiredBoolean(parent: GddlMap, key: String, path: String): Boolean {
        ...
        if (!value.isBoolean) throw InvalidConfigException("Expected property $path.$key to be a boolean, but got $value")
        return value.booleanValue()
    }

    private fun requiredInteger(parent: GddlMap, key: String, path: String): BigInteger {
        val value = parent.get(key) ?: throw InvalidConfigException("Missing required integer property $path.$key")
        if (value.isInteger) return BigInteger.valueOf(value.toLong())
        if (value.isString) {
            try {
                return BigInteger(value.stringValue())
            } catch (invalid: NumberFormatException) {
                throw InvalidConfigException("Property $path.$key (${value.stringValue()}) is not a valid integer")
            }
        }
        if (value.isDouble) {
    ...
    private fun loadArea(properties: GddlMap, path: String) = AreaClass(
        ...
        displacementClassName = requiredString(properties, "displacement", path),
        createNumberExtensions = requiredBoolean(properties, "createNumberExtensions", path)
    )
    ...
    private fun parseModule(): FixieModule {
