...
class FixieModule(
	...
	private inline fun <reified S : QuantityClass, reified T, R> checkPresent(
		...
		for (element in sourceElements) {
			...
			val extractTarget = if (QuantityClass::class.java.isAssignableFrom(T::class.java)) { quantity: T ->
				(quantity as QuantityClass).className
			} else { target: T ->
				target
			}
	...
	init {
		...
		resolve(volumes, displacements, { it.displacementClassName }) { volume, displacement ->
			volume.displacement = displacement
		}
		resolve(areas, volumes, { it.volumeClassName }) { area, volume -> area.volume = volume }
		resolve(areas, displacements, { it.displacementClassName }) { area, displacement ->
