...
internal class AngleClassGenerator(
	...
	override fun generateArithmetic() {
		...
		if (quantity.allowDivisionAndFloatMultiplication) {
			...
			writer.println()
			writer.println(
				"\toperator fun div(right: Long) = ${quantity.className}(${
					conversion(
						"this.raw / right$fixSign",
						8
					)
				})"
			)
			...
			quantity.spinClass?.let { spin ->
				writer.println()
				val angleUnit =
					if (spin.oneUnit == SpinUnit.DEGREES_PER_SECOND) AngleUnit.DEGREES else AngleUnit.RADIANS
		...
		writer.println()
		writer.println(
			"\toperator fun times(right: Long) = ${quantity.className}(${
				conversion(
					"this.raw * right$fixSign",
					8
				)
	...
	override fun generateCompanionContent() {
		...
		for (unit in AngleUnit.entries) {
			...
			if (quantity.internalType.numBytes == 8) {
				...
				val signedConversion =
					"raw(((value % ${unit.maxValue}) * $factor).roundToLong().to${quantity.internalType}())"
