...
import fixie.generator.bigIntegerToULong
...
internal class NumberTestsGenerator(
	...
	private fun generateTestSequence(
		...
		while (value < maxValue / BigInteger.valueOf(growFactor.toLong()) && value < maxValue) {
			if (value != minValue) {
				...
				for (counter in 0 until density) {
					val bound1 =
						(value.toBigDecimal() / marginFactor + BigDecimal.valueOf(0.5 * value.signum())).toBigInteger()
					val bound2 =
						(value.toBigDecimal() * marginFactor + BigDecimal.valueOf(0.5 * value.signum())).toBigInteger()
			...
			else if (value > BigInteger.valueOf(growFactor.toLong() * growFactor)) value *= BigInteger.valueOf(
				growFactor.toLong()
			)
	...
	private fun generateDoubleConversion(convertToFloat: Boolean) {
		...
		if (convertToFloat) {
			...
			while (maxValue * number.oneValue.toDouble() > number.internalType.getMaxValue().toDouble()) maxValue =
				maxValue.nextDown() * 0.999
		}
	...
	private fun generateAdditionAndSubtraction() {
		...
		val oneLongValue = if (number.internalType.signed) number.oneValue.longValueExact().toString()
		else bigIntegerToULong(number.oneValue).toString() + "u"
			...
			for (candidate in intSequence2) {
				...
				for (adder in arrayOf(adder1, adder2)) {
					...
					if (BigInteger.valueOf(candidate.toLong()) == candidate &&
						...
						val longAdderString =
							if (adder == BigInteger.valueOf(Long.MIN_VALUE)) "Long.MIN_VALUE" else "${adder}L"
				...
				for (difference in arrayOf(difference1, difference2)) {
					...
					if (BigInteger.valueOf(difference.toLong()) == difference &&
						...
						val longDifferenceString =
							if (difference == BigInteger.valueOf(Long.MIN_VALUE)) "Long.MIN_VALUE" else "${difference}L"
		...
		val almostOneValue = number.oneValue.subtract(BigInteger.ONE)
		val raw = BigInteger.ONE + if (number.internalType.signed) {
			BigInteger.valueOf(rng.nextLong(almostOneValue.longValueExact()))
		} else uLongToBigInteger(rng.nextULong(bigIntegerToULong(almostOneValue)))
	...
	private fun generateMultiplicationAndDivision() {
		...
		fun randomValue() = if (number.internalType.signed) {
			BigInteger.valueOf(rng.nextLong(number.oneValue.longValueExact() / 2, number.oneValue.longValueExact()))
		} else {
			uLongToBigInteger(
				rng.nextULong(
					bigIntegerToULong(number.oneValue / BigInteger.TWO),
					bigIntegerToULong(number.oneValue)
				)
			)
		}

		var random1 = -randomValue()
		val random2 = randomValue()
		if (!number.internalType.signed) random1 = -random1

		val suffix = if (number.internalType.signed) "" else "u"
		val maxDifference = BigInteger.TWO.max(number.oneValue / BigInteger.valueOf(100))
		writer.println(
			"\t\tassertEquals(${number.className}.raw($random1$suffix), " +
					"(${number.className}.raw($random1$suffix) * ${number.className}.raw($random2$suffix)) / " +
					"${number.className}.raw($random2$suffix), ${number.className}.raw($maxDifference$suffix))"
		)
	...
	private fun generateCompareTo() {
		...
		writer.println(
			"\t\tassertTrue(${number.className}.raw(${number.internalType}.MAX_VALUE) < ${
				maxIntValue.add(
					BigInteger.ONE
				)
			}${if (number.internalType.signed) "" else "u"})"
		)
		writer.println(
			"\t\tassertTrue(${number.className}.raw(${number.internalType}.MAX_VALUE) < ${
				maxIntValue.add(
		...
		if (number.internalType.signed) {
			...
			writer.println(
				"\t\tassertTrue(${number.className}.raw(${number.internalType}.MIN_VALUE) > ${
					minIntValue.subtract(
						BigInteger.ONE
					)
				})"
			)
