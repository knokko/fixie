...
internal class SpeedTestsGenerator(
    ...
    private val suffix = if (speed.number != null) { if (speed.number.internalType.signed) "" else "u" } else {
        if (speed.floatType == FloatType.SinglePrecision) "f" else ".0"
    }
    ...
    private fun shouldTestSeconds() = speed.number == null || speed.number.oneValue * BigInteger.valueOf(100L) < speed.number.internalType.getMaxValue()

    private fun shouldTestHours() = speed.number == null || speed.number.oneValue * BigInteger.valueOf(100_000L) < speed.number.internalType.getMaxValue()
    ...
    private fun generateEquals() {
        ...
        if ((speed.number == null || speed.number.oneValue > BigInteger.TEN) && speed.oneUnit.ordinal > 0) {
        ...
        if ((speed.number == null || speed.number.oneValue * BigInteger.TEN < speed.number.internalType.getMaxValue()) && speed.oneUnit != SpeedUnit.entries.last()) {
    ...
    private fun generateToDouble() {
        ...
        val testValue = if (speed.number == null || speed.number.oneValue > BigInteger.TEN) 0.7 else 7.0
        val margin = if (speed.number != null && speed.number.oneValue < BigInteger.TEN) 0.4
        else if (speed.number != null && speed.number.oneValue < BigInteger.valueOf(1000L)) 0.04 else 0.001
    ...
    private fun generateCompanionConstructors() {
        ...
        if (speed.number == null || speed.number.oneValue > BigInteger.TEN) {
    ...
    private fun generateArithmetic() {
        ...
        if (speed.number == null || speed.number.oneValue * BigInteger.valueOf(21L) < speed.number.internalType.getMaxValue()) {
        ...
        if (speed.number == null || speed.number.oneValue * BigInteger.valueOf(3) < speed.number.internalType.getMaxValue()) {
            if (speed.number != null) {
        ...
        if (speed.number == null || speed.number.internalType.signed) {
        ...
        if (shouldTestSeconds()) {
            ...
            val isDisplacementAccurate = speed.displacementNumber?.oneValue == null || speed.displacementNumber.oneValue > BigInteger.TEN
    ...
    private fun generateMathFunctions() {
        ...
        if (speed.number?.internalType?.signed != false) {
            writer.println("\t\tassertEquals(${speed.className}.raw(0$suffix), abs(${speed.className}.raw(0$suffix)))")
            writer.println("\t\tassertEquals(${speed.className}.raw(5$suffix), abs(${speed.className}.raw(-5$suffix)))")
            writer.println("\t\tassertEquals(${speed.className}.raw(5$suffix), abs(${speed.className}.raw(5$suffix)))")
        }
